...
<script type="text/ecmascript-6">
  ...
  export default {
    props: {
      ...
      // 重新计算高度的方法，的延迟事件；
      // 如果父级不传入,那么执行重新计算函数的延迟就是默认20毫秒;
      // 这里是因为某些父级的组件有动画,会超过20毫秒,所以如果在
      // 这个父级的动画完成后,再重新计算高度的话,
      // 那么其实是没有把删除或添加的DOM元素的高度计算在内容的
      // 比如:父级添加或删除某个元素,动画时间是100毫秒,但是这个scroll组件的重新计算是20毫秒后就执行了
      // 所以在执行完重新计算函数后,动画还没有结束,也就是DOM还有没有除以或添加完,此时的DOM高度计算是有误的;
      // 所以这里用props监听父级传入的延迟时间,如果父级没有动画的话,也就不需要传入延迟时间,用默认的20毫秒就行;
    ...
    watch: {
      ...
      data() {
        setTimeout(() => {
          this.refresh()
          // 这里的refreshDelay延迟时间,是由父级传入,用props监听到的
          // 如果父级对重新计算的延迟时间没有要求,就不需要传入,用默认的20毫秒即可
          // 也就是说,scroll组件监听到的父级data有变化,
          // 当动画完成后,DOM的高度才是正确的,这时再调用scroll组件的重新计算方法;
        }, this.refreshDelay)
      }
    }
  }
